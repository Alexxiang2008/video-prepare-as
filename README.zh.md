...
video-prepare-as 就是来陪你过这一关的。装好之后,你在 Codex 或者 Claude Code 里说一句"我想做个视频",它就接管对话,像编导听你讲 brief 那样一路追问:这视频给谁看?多长?最想让人记住哪句话?哪个镜头是重点?你答不上来的、压根没想到的地方,它会停下来提醒你、帮你补上。
...
动手之前,先把两样东西装好:HyperFrames(下游负责渲染)和 video-prepare-as(这个 skill 本身)。都用 `skills` 这个命令行工具装,各一条命令:
